...
const ImageSlider = ({ casualPicks }) => {
  ...
  const movies = [
    "65",
    ["Wednesday", "Wonder Woman", "Ghosted", " "],
    "Ant-Man",
    "",
    "     ",
    "Party, Animal Kingdom, Under the skin,  ",
    "Lady Bird, Moon, Seven kings must die",
    "Juicy World",
    ["Platton", "Wall-E"],
    {
      one: 'John Wick',
  ...
  const arr = movies.flatMap((each) => {
    if (Array.isArray(each)) {
      return each.map((one) => one.trim()).filter(one => one !== '');
    } else if (typeof each === "string" && each.trim() === "") {
      return [];
    } else if(typeof each === "object" ) {
       const arr2 =  Object.entries(each)
       return arr2.flatMap(each2 => {
        return each2
       })
    } else {
      return each
